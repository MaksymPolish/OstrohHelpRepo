...
Чат побудований як поєднання REST API (для завантаження історії і відправки) та SignalR (для real-time оновлень).

### 1) Завантаження списку чатів
- Після авторизації фронтенд отримує консультації користувача через:
	- `GET /Consultations/Get-All-Consultations-By-UserId/{userId}`
- На основі `studentId` / `psychologistId` визначається співрозмовник (peer), його ім'я та аватар.

### 2) Завантаження історії повідомлень
- Для вибраної консультації виконується:
	- `GET /Message/Recive?idConsultation={consultationId}`
- Фронтенд нормалізує відповіді (camelCase/PascalCase, вкладені DTO) та сортує повідомлення від старих до нових.

### 3) Підключення до SignalR
- Клієнт відкриває з'єднання до `REACT_APP_SIGNALR_HUB_URL` з JWT токеном.
- Після вибору чату викликається `JoinConsultation(consultationId)`.
- При вході в консультацію сервер надсилає `ReceiveConsultationKey` (ключ шифрування в Base64).
- Ключ зберігається тільки в RAM (in-memory), без localStorage.

### 4) Шифрування повідомлень на клієнті
- Перед відправкою текст шифрується на фронтенді (`src/services/encryptionService.js`).
- У payload повідомлення передаються:
	- `senderId`
	- `receiverId`
	- `consultationId`
	- `encryptedContent`
...
- Основний endpoint:
	- `POST /Message/Send`
- Для вкладень використовується окремий upload flow, після чого URL файлів додаються до повідомлення.

### 6) Отримання нових повідомлень у real-time
- SignalR подія `ReceiveMessage` доставляє нові повідомлення в активний чат.
- Якщо повідомлення зашифроване, фронтенд виконує дешифрування локально.
- При помилці верифікації `authTag` повідомлення не розшифровується як валідний plaintext.

### 7) Відмовостійкість
- Якщо SignalR тимчасово недоступний, історія все одно працює через REST.
- Для авторизації API-клієнт підтримує refresh токена (коли ввімкнено через env).
- При 401 і неможливості refresh, сесія очищається і виконується редирект на login.
